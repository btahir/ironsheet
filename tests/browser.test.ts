...
test("browser compression adapter deflates and inflates ZIP entries", async () => {
  const zip = await writeZip(
    [{ name: "xl/workbook.xml", data: textEncoder.encode("browser-ready"), compressionMethod: 8 }],
