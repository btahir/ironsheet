...
The site deploys to Vercel: set the project's Root Directory to `website/` and the defaults work — the generated API reference is committed, so the deploy build needs no repo-root dependencies. Set `NEXT_PUBLIC_SITE_URL` (or rely on Vercel's production URL) for canonical/OG metadata.
